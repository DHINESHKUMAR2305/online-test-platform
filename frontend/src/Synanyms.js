...
const Synanyms = ({ synanyms }) => {
    const [synanymsanswer, setsynanymsanswer] = useState({})

    const handlechange = (id, value) => {
        setsynanymsanswer((prev) =>
        ({
            ...prev,
            [id]: value
        }))
    ...
    const checkscoreforsynanyms = async () => {
        const res = await axios.post("https://online-test-backend-ngjo.onrender.com/synonyms/synanymsanswer", { answer: synanymsanswer })
    ...
    return (
        <div className="synanyms">
            <h1 className="title">Synonyms</h1>
            {synanyms.map((xx, value) =>
            (
